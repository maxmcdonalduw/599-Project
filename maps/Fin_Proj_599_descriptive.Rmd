...
# region boundaries
temp_honduras_data <- honduras_data[, c('region', 'DHSCLUST')]
temp_honduras_data <- unique(temp_honduras_data)

# wealth categories
honduras_wc <- honduras_data[, c('DHSCLUST', 'wealthCat')]
honduras_wc <- unique(honduras_wc)


DFW <- merge(x=geodata_spatial, y=temp_honduras_data, by.x = "DHSCLUST", by.y = "DHSCLUST")
DFW
...
agg_honduras <- aggregate(list(category_count = honduras_data$wealthCat), by = list(cluster = honduras_data$DHSCLUST, wealthcat = honduras_data$wealthCat), FUN = length)

agg_reshape <- reshape(data = agg_honduras, direction = 'wide', idvar = 'cluster', timevar = 'wealthcat')

new_honduras <- merge(x=DFW, y=agg_reshape, by.x = "DHSCLUST", by.y = "DHSCLUST")
